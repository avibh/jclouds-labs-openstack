...
public class SampleApiLiveTest extends BaseCeilometerApiLiveTest {
   ...
   public void testListSamplesWithQuery() {
      for (String region : filterRegions(api.getConfiguredRegions())) {
         QueryApi queryApi = api.getQueryApi("RegionOne");
         ...
         String filter = "{\"and\": [{\">=\": {\"timestamp\": \"" + dateFormat.format(calendarFrom.getTime())
               + "\"}}, {\"<=\": {\"timestamp\": \"" + dateFormat.format(calendarTo.getTime()) + "\"}}]}";

         List<Sample> samples = queryApi.listSamples(ComplexQueryOptions.builder().filter(filter).limit(5).build());

         assertThat(samples).isNotEmpty();
         assertThat(samples.size()).isEqualTo(5);
